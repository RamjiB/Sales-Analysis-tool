...
def _normalized_text_expr(quoted_col: str) -> str:
    # Trim whitespace and treat empty strings as NULL so "unique" values are stable.
    return f"NULLIF(TRIM(CAST({quoted_col} AS VARCHAR)), '')"
...
def _collect_low_cardinality_categorical_values(
    ...
    for row in rows:
        ...
        norm = _normalized_text_expr(quoted_col)
        non_null_count, distinct_count = con.execute(
            f"""
            SELECT
              COUNT(*) FILTER (WHERE {norm} IS NOT NULL) AS non_null_count,
              COUNT(DISTINCT {norm}) AS distinct_count
            FROM {quoted_table}
            """
        ).fetchone()
        ...
        if non_null == 0:
            continue
        if max_distinct > 0 and distinct > max_distinct:
            continue
        if (distinct / non_null) >= threshold_ratio:
            continue

        limit_sql = "" if max_values_per_column <= 0 else f"LIMIT {int(max_values_per_column)}"
        values = con.execute(
            f"""
            SELECT {norm} AS value
            FROM {quoted_table}
...
def run_ingestion(record: dict, progress_cb=None) -> IngestionResponse:
    ...
    categorical_unique_values = {
        "threshold_ratio": settings.categorical_hint_threshold_ratio,
        "max_distinct": settings.categorical_hint_max_distinct,
        "max_values_per_column": settings.categorical_hint_max_values,
        "columns": {},
    }
    with duckdb.connect(str(settings.duckdb_path)) as con:
        ...
        try:
            categorical_unique_values = _collect_low_cardinality_categorical_values(
                con,
                record["table_name"],
                threshold_ratio=settings.categorical_hint_threshold_ratio,
                max_distinct=settings.categorical_hint_max_distinct,
